...
def distance_to_buffer(current_trajectory, trajectory_buffer, h=3, gap=3):
    """
    ...
    # the length of preferred trajectories
    num = len(trajectory_buffer)
    
    for i, expert_trajectory in enumerate(trajectory_buffer):
        distance = MMD_calculate(current_trajectory, expert_trajectory, h=h, gap=gap)
        if distance > max_distance:
            max_distance = distance
            max_trajectory = expert_trajectory
            max_idx = i
