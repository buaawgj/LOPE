...
class Monitor(Wrapper):
    ...
    def __init__(
        ...
        basic_keywords=(
            'r', 'l', 't', 'reward_ctrl', 'goal', "inner_rew",
            'com', 'ori', 'x_position', 'x_velocity'
            ), 
        info_keywords=(
            'aver_return_iter', 'aver_return_smooth', 
            'success_rate', 'aver_mmd_distance',
            ),
        extra_keywords=('num_goal',),
        use_sparse=False,
        ):
        Wrapper.__init__(self, env=env)
        self.env = env
        self.tstart = time.time()
        folder_name = "Hopper_lope_" + time.strftime("%d-%m-%Y_%H-%M-%S") 
        ...
        self.episode_returns = deque(maxlen=40)
    ...
    def step(self, action):
        ...
        ob, rew, done, _, infos = self.env.step(action)
        
        infos["goal"] = False
        if self.use_sparse:
            infos['inner_rew'] = rew
            if infos['x_position'] < 1:
                rew = 0.
            if infos['x_position'] >= 1:
                rew = 1.5
            if infos['x_position'] >= 3.5:
                rew = 2.5
            if infos['x_position'] >= 6:
                rew = 3.5
            if infos['x_position'] >= 8:
                rew = 4.5
                # infos['goa'] = True
    ...
    def update(self, ob, rew, done, infos):
        ...
        success_rate = 0.
        self.rewards.append(infos['inner_rew'])
        # self.rewards.append(rew)
        if done or len(self.rewards) == self.max_ep_len:
            self.needs_reset = True
            eprew = np.mean(self.rewards)
            eplen = len(self.rewards)
            epret = sum(self.rewards)
            
            self.episode_rewards.append(eprew)
            self.epres.append(eprew)
            self.episode_returns.append(epret)
    ...
    def write_data(self, epinfos, mmd_distances):
        ...
        for idx, epinfo in enumerate(epinfos):
            ...
            if self.episode_num % self.update_freq == 0:
                ...
                perform_info["num_goal"] = num_goal
